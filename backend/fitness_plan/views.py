...
class DeleteFitnessProfileView(APIView):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]
    ...
    def delete(self, request, *args, **kwargs):
        try:
            ...
            # Delete associated Task and WorkoutPlan
            if fitness_profile.workout_plan:
                fitness_profile.workout_plan.tasks.all().delete()
                fitness_profile.workout_plan.delete()

            # Delete FitnessProfile
            fitness_profile.delete()

            return Response({'detail': 'FitnessProfile and associated data deleted successfully'}, status=status.HTTP_204_NO_CONTENT)

        except FitnessProfile.DoesNotExist:
            return Response({'detail': 'FitnessProfile not found'}, status=status.HTTP_404_NOT_FOUND)
